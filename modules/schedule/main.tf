...
#
# ⚠️ Karpenter 빈틈 (팀 결정 2026-07-21 때 함께 짚은 전제):
#    02:00 에 시스템 노드그룹이 0 이 되면 그 위의 Karpenter 컨트롤러도 죽는다.
#    그 시점에 살아 있던 Spot 노드는 회수 주체가 없어 아침까지 남는다.
#    전제 - 밤에는 큐가 비어 KEDA 가 worker 를 줄이고 Karpenter 가 빈 노드를 먼저 회수한다.
#    잔존해도 Spot 단가라 손실이 작고, 10:00 컨트롤러 부활 뒤 회수된다.
#    Lambda 로 강제 정리하는 확장은 하지 않는다(YAGNI).
