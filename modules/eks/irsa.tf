...
#   calls/*               |    -    |   읽기   | 읽기+쓰기 |   -
#
# ⚠️ 앱의 FileStore 는 read 하기 전에 exists() 를 먼저 부르고, exists() 는 head_object 다
...
# ⚠️ 운영에서 CreateBucket 은 안 나간다. 어댑터의 auto_create 가 전 서비스 기본 false 이고
...
data "aws_iam_policy_document" "predict" {
  ...
  #                  ⚠️ latest.json 하나로 좁히면 안 된다 — history/<타임스탬프>.json 이 함께 올라간다
  ...
  #
  # ⚠️ 미상(unknown) ARN 이 와도 plan 은 죽지 않는다. 다만 이유를 정확히 알아 둬라.
  ...
  #
  #    ⚠️ 이 논리를 '리소스 레벨' for_each/count 에 그대로 옮기지 마라 —
...
#    — "weather forecast CSV empty/missing" 로 죽는다).
#
