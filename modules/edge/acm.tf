...
#   ① apex(hailcast.myminiinfra.store)와 wildcard(*.hailcast.myminiinfra.store)의 검증 CNAME 이 같다.
...
#
# for_each 를 쓰면 안 되는 이유 (2026-07-16 · 이미선 리뷰 지적 · 옛 코드의 버그).
...
#
# 전제: SAN 이 apex + wildcard 뿐일 때만 성립한다.
#   나중에 SAN 에 다른 이름(예: api.hailcast.myminiinfra.store)을 더하면 그 도메인은 '다른 CNAME' 을 받아
...
# 이게 없으면 인증서가 PENDING_VALIDATION 인 채로 ALB 리스너·CloudFront 가 붙으려다
